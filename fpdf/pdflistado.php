...
function addDate( $date )
{
	...
	$this->Cell(10,0, "FECHA PEDIDO", 0, 0, "C");
...
function addClient($cliente)
{
	...
	$this->SetXY($r1 + ($r2 - $r1) / 2 - 5, $y1 + 3);
	$this->SetFont("Arial", "B", 10);
	$this->Cell(10, 0, utf8_decode("CLIENTE"), 0, 0, "C");
	$this->SetXY($r1 + ($r2 - $r1) / 2 - 5, $y1 + 9);
	$this->SetFont("Arial", "", 10);
	$this->Cell(10, 0, utf8_decode($cliente), 0, 0, "C");
}
...
function addFormaPago($formapago)
{
	...
	$this->SetXY($r1 + ($r2 - $r1) / 2 - 5, $y1 + 3);
	$this->SetFont("Arial", "B", 10);
	$this->Cell(10, 0, utf8_decode("FORMA DE PAGO"), 0, 0, "C");
	$this->SetXY($r1 + ($r2 - $r1) / 2 - 5, $y1 + 9);
	$this->SetFont("Arial", "", 10);
	$this->Cell(10, 0, utf8_decode($formapago), 0, 0, "C");
}
...
function addLeyenda( $tot )
{
	$r1 = 10;
	$y1 = 260;
	$this->SetXY( $r1, $y1);
	$this->SetFont( "Arial", "B", 11);
	$this->MultiCell( 200, 4, utf8_decode($tot));
}

function addDireccion($zona,$direccion)
{
		$r1  = 66;
		$r2  = $r1 + 133;
		$y1  = 27;
		...
		$this->SetXY($r1 + ($r2 - $r1) / 2 - 5, $y1 + 3);
		$this->SetFont("Arial", "B", 10);
		$this->Cell(10, 0, utf8_decode("DIRECCIÓN: ". strtoupper($zona)), 0, 0, "C");
		$this->SetXY($r1 + ($r2 - $r1) / 2 - 5, $y1 + 9);
		if ($length = $this->GetStringWidth(utf8_decode($direccion)) > 145) {
			$this->SetFont("Arial", "", 8);
		}else {
			$this->SetFont("Arial", "", 10);
		}
		$this->Cell(10, 0, utf8_decode($direccion), 0, 0, "C");
}

function addCantidad($cantidad)
{
		$r1  = 45;
		$r2  = $r1 + 20;
		$y1  = 27;
		...
		$this->SetXY($r1 + ($r2 - $r1) / 2 - 5, $y1 + 3);
		$this->SetFont("Arial", "B", 10);
		$this->Cell(10, 0, utf8_decode("ITEMS"), 0, 0, "C");
		$this->SetXY($r1 + ($r2 - $r1) / 2 - 5, $y1 + 9);
		if ($length = $this->GetStringWidth(utf8_decode($cantidad)) > 145) {
			$this->SetFont("Arial", "", 8);
		}else {
			$this->SetFont("Arial", "", 10);
		}
		$this->Cell(10, 0, utf8_decode($cantidad), 0, 0, "C");
}
function addComentarios($coment)
{
		$r1  = 10;
		$r2  = $r1 + 190;
		$y1  = 42;
		...
		$this->SetXY($r1 + ($r2 - $r1) / 2 - 5, $y1 + 3);
		$this->SetFont("Arial", "B", 10);
		$this->Cell(10, 0, utf8_decode("COMENTARIOS"), 0, 0, "C");
		$this->SetXY($r1 + ($r2 - $r1) / 2 - 5, $y1 + 9);
		$this->SetFont("Arial", "", 10);
		
		$this->Cell(10, 0, utf8_decode($coment), 0, 0, "C");
}
function addFechaEntrega($fecha)
{
		$r1  = 10;
		$r2  = $r1 + 34;
		$y1  = 27;
		...
		$this->SetXY($r1 + ($r2 - $r1) / 2 - 5, $y1 + 3);
		$this->SetFont("Arial", "B", 10);
		$this->Cell(10, 0, utf8_decode("FECHA ENTREGA"), 0, 0, "C");
		$this->SetXY($r1 + ($r2 - $r1) / 2 - 5, $y1 + 9);
		$this->SetFont("Arial", "", 10);
		$this->Cell(10, 0, utf8_decode($fecha), 0, 0, "C");
}
