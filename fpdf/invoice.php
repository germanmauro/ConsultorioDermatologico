...
function addSociete( $nom, $adresse )
{
	...
	$this->SetXY( $x1+5, $y1 + 15 );
	$this->MultiCell($length, 4, utf8_decode("Teléfonos:\n 0220-4946163 15-5229-3448"));
	$this->SetXY( $x1, $y1 + 24 );
	$this->MultiCell($length, 4, utf8_decode("www.materales-licata.com.ar"));
	$this->Image('../whatsapp.png',10,24,5,5);
	$this->Image('../LogoLicata.png',65,5,-1000);
}
...
function addFirma( )
{
	...
	$y1     = 270;
	$this->SetXY( $r1, $y1);
	$this->MultiCell( 60, 4, utf8_decode("ACLARACIÓN:"));
}
...
function addClientAdresse( $adresse )
{
	...
	$this->SetFont( "Arial", "B", 10);
	$this->SetXY( $r1, $y1);
	$this->MultiCell( 180, 4, utf8_decode("Dirección de entrega: ".$adresse));
}

// Fecha
function addDescripcion( $adresse )
{
	$r1     = $this->w - 199;
	//$r2     = $r1 + 68;
	$y1     = 58;
	...
	$this->SetXY( $r1, $y1);
	$this->MultiCell( 180, 4, "<<  ".utf8_decode($adresse)."  >>");
}
...
function addTotal( $tot )
{
	...
	$this->SetXY( $r1, $y1);
	$this->SetFont( "Arial", "B", 12);
	$this->MultiCell( 60, 4, utf8_decode("TOTAL $".$tot));
}
//Intereses
function addInteres( $tot, $cuota )
{
	$r1     = $this->w - 110;
	$y1     = 260;
	$this->SetXY( $r1, $y1);
	$this->SetFont( "Arial", "B", 11);
	$this->MultiCell( 120, 4, utf8_decode("INTERÉS:".$cuota." $".$tot));
}
//SubTotal
function addSubTotal( $tot )
{
	$r1     = $this->w - 55;
	$y1     = 250;
	$this->SetXY( $r1, $y1);
	$this->SetFont( "Arial", "B", 11);
	$this->MultiCell( 60, 4, utf8_decode("SUBTOTAL $".$tot));
}
...
function addPago( $pago )
{
	...
	$this->SetXY( $r1 + ($r2 - $r1)/2 - 5 , $y1+1 );
	$this->SetFont( "Arial", "B", 10);
	$this->Cell(10,4, "PAGO", 0, 0, "C");
	$this->SetXY( $r1 + ($r2-$r1)/2 - 5 , $y1 + 5 );
	$this->SetFont( "Arial", "B", 11);
	$this->Cell(10,5,$pago, 0,0, "C");
}

// VAT number
function addVendedor($tva)
{
	$r1  = 10;
	$r2  = $r1 + 80;
	$y1  = 40;
	...
	$this->SetXY( $r1 + ($r2-$r1)/2 -5 , $y1+1 );
	$this->SetFont( "Arial", "B", 9);
	$this->Cell(10,4, "VENDEDOR", 0, 0, "C");
	$this->SetXY( $r1 + ($r2-$r1)/2 -5 , $y1 + 5 );
	$this->SetFont( "Arial", "", 9);
	$this->Cell(10,5,$tva, 0,0, "C");
}

function addEstado($ref)
{
	$this->SetFont( "Arial", "", 10);
	$length = $this->GetStringWidth(utf8_decode("ESTADO: ".$ref));
	$r1  = 10;
	$r2  = $r1 + $length;
	$y1  = 85;
	$y2  = $y1+5;
	$this->SetXY( $r1 , $y1 );
	$this->SetFont( "Arial", "B", 10);
	$this->Cell($length,4,utf8_decode("ESTADO: <<".$ref.">>"));
}
